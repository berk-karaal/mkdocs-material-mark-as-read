...
class MarkAsReadPlugin(BasePlugin[MarkAsReadConfig]):
    ...
    def on_page_markdown(
        ...
        markdown += (
            f"\n\n:{self.config['read_mark']}:{{.mark-as-read-display-none .mark-as-read-icon #mark-as-read-read-icon title='Read'}}"
            f"\n:{self.config['updated_mark']}:{{.mark-as-read-display-none .mark-as-read-icon #mark-as-read-updated-icon title='Read but updated'}}"
        )
